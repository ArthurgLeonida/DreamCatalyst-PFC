...
"""
Method configurations — trimmed for DreamCatalyst-NS.
Only Splatfacto variants are kept (no NeRF/NeuS/InstantNGP/etc.).
"""
...
descriptions = {
    ...
    "splatfacto-mcmc": "Splatfacto with MCMC densification strategy.",
}
...
def discover_methods():
    """Discover externally registered methods (e.g. dream-catalyst via entry_points)."""
    try:
        from nerfstudio.plugins.registry import discover_methods as _discover
        return _discover()
    except ImportError:
        # Plugins module removed in trimmed build — discover via importlib directly
        pass
    ...
    try:
        ...
        if hasattr(eps, "select"):
            ns_eps = eps.select(group="nerfstudio.method_configs")
        else:
            ns_eps = eps.get("nerfstudio.method_configs", [])
        for ep in ns_eps:
            try:
                spec = ep.load()
                methods[ep.name] = spec.config
                descs[ep.name] = spec.description
            except Exception:
                pass
    except Exception:
        pass
...
# Add discovered external methods (e.g. dream-catalyst)
