...
6. **Depth-Masked Perp-Neg** [DONE] — spatially restrict the Gram-Schmidt projection to foreground pixels, preserving background identity.
   ...
   * **Implementation:** `dc_pipeline.py` extracts `camera_outputs["depth"]`, normalizes to [0,1] via min-max, binarizes at threshold, resizes to 512-space, passes to `dc.py`. Inside the Perp-Neg block, mask is resized to match `tgt_x0.shape[2:]` (robust to VAE rounding). Uses the same **global projection scalar** as standard Perp-Neg (stable — computed over all ~22K latent elements), but gates the subtraction spatially: `eps_tgt = eps_tgt - projection * eps_src * mask`. Background (mask=0) keeps original `eps_tgt` untouched.
   * **Why global projection + spatial mask (not per-pixel):** per-pixel projection (`sum over dim=1` = 4 channels) is extremely noisy — produces salt-and-pepper gradient artifacts. Global projection is stable and the mask handles spatial selectivity.
